...
**D. Validação de Qualidade (Linting)**

```yaml
      - name: Verificar Estilo (Linting)
        run: flake8 src/ tests/ --count --statistics
```

* **Função**: Analisa se o código segue os padrões de escrita (PEP8) e se não há erros de sintaxe ou variáveis não usadas.
...
* **Função**: Executa o script que gera o modelo (`model.pkl`). Isso garante que o modelo em produção foi gerado pelo código atual.
...
* **Função**: Executa testes unitários na API e no modelo. Se falhar, o pipeline para.
...
* **Função**: Cria a imagem Docker contendo a API e o modelo treinado, pronta para deploy.
